...
func TestRepoInterface(t *testing.T) {
	var _ interface {
		...
		GetCustomerByPhone(context.Context, uuid.UUID, string) (*auth.Customer, error)
		...
		GetCustomerIdentity(context.Context, uuid.UUID, string, string) (*auth.CustomerIdentity, error)
